...
export const getPlants = async ({
  ...
  const query = PlantCollection.find(filters, { name: 1, photo: 1 })
    .skip(skip)
    .limit(perPage)
    .sort({ [sortBy]: sortOrder as SortOrderType })
    .lean();
...
export const getNameList = async () =>
  await PlantCollection.find({}, { name: 1 }).lean();
...
export const togglePlantFavorite = async (
  plantId: string,
...
export const updatePlant = async ({
  ...
  const rawResult = await PlantCollection.findOneAndUpdate(
    { _id },
